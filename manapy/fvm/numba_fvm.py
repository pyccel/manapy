...
@njit
def compute_upwind_flux(w_l:'float', w_r:'float', u_face:'float', v_face:'float', normal:'float[:]', flux_w:'float'):
    
    sol = 0.
    sign = u_face * normal[0] + v_face * normal[1]
...
@njit
def explicitscheme_convective_2d(rez_w:'float[:]', w_c:'float[:]', w_ghost:'float[:]',  w_halo:'float[:]', u_face:'float[:]', v_face:'float[:]', 
                                 w_x:'float[:]', w_y:'float[:]', wx_halo:'float[:]', wy_halo:'float[:]', psi:'float[:]', psi_halo:'float[:]', 
                                 centerc:'float[:,:]', centerf:'float[:,:]', centerh:'float[:,:]', 
                                 centerg:'float[:,:]', cellidf:'int[:,:]', mesuref:'float[:]', normalf:'float[:,:]', halofid:'int[:]', name:'int[:]',
                                 shift:'float[:,:]', order:'int'):
    ...
    for i in range(nbface):
        ...
        if name[i] == 0: 
            ...
            w_ln  = w_l  + (order - 1) * psi_left  * (w_x_left * r_l[0]  + w_y_left * r_l[1] )
            w_rn  = w_r  + (order - 1) * psi_right * (w_x_right* r_r[0]  + w_y_right* r_r[1] )
        ...
        flux_w = 0.    
        compute_upwind_flux(w_ln, w_rn, u_face[i], v_face[i], normal, flux_w)
        
        rez_w[cellidf[i][0]]      -= flux_w
          
        if name[i] == 0:
            rez_w[cellidf[i][1]]  += flux_w
...
def explicitscheme_convective_3d(rez_w:'float[:]', w_c:'float[:]', w_ghost:'float[:]', w_halo:'float[:]',
    ...
    for i in range(nbface):
        ...
        if name[i] == 0:
            ...
            w_ln  = w_l  + (order - 1) * psi_left  * (w_x_left * r_l[0]  + w_y_left * r_l[1] + w_z_left * r_l[2] )
            w_rn  = w_r  + (order - 1) * psi_right * (w_x_right* r_r[0]  + w_y_right* r_r[1] + w_z_right * r_r[2] )
        ...
        sign = u_face[i] * normal[0] + v_face[i] * normal[1] + w_face[i] * normal[2]
